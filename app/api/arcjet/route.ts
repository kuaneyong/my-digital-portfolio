...
async function handleDecision(req: Request) {
  ...
  if (decision.isDenied()) {
    if (decision.reason.isRateLimit()) return jsonResponse({ error: 'Too many requests' }, 429);
    if (decision.reason.isBot()) return jsonResponse({ error: 'No bots allowed' }, 403);
    return jsonResponse({ error: 'Forbidden' }, 403);
  }

  if (decision.ip?.isHosting()) return jsonResponse({ error: 'Forbidden' }, 403);
  if (decision.results?.some(isSpoofedBot)) return jsonResponse({ error: 'Forbidden' }, 403);
